...
struct SyncDefaultRepositoryTests {
	...
	@Test func pull_callsRemoteWithCorrectRequest() async throws {
		let remote = SyncMockRemoteDataSource(pullResult: .success(anyPullSuccessResponse()))
		let sut = makeSUT(remote: remote)

		_ = try await sut.pull(request: anyPullRequest())

		#expect(remote.invocations == [.pull(anyPullRequest())])
	}

	@Test(arguments: [0, 1, 2])
	func pull_withVaryingUserCategoryCounts_returnsMatchingList(itemCount: Int) async throws {
		let items = (0..<itemCount).map { anyUserCategoryItem(id: "id\($0)") }
		let remote = SyncMockRemoteDataSource(pullResult: .success(anyPullSuccessResponse(userCategories: items)))
		let sut = makeSUT(remote: remote)

		let result = try await sut.pull(request: anyPullRequest())

		guard case .loaded(let response) = result else {
			Issue.record("Expected .loaded but got \(result)")
			return
		}
		#expect(response.data?.userCategories?.count == itemCount)
	}

	@Test func pull_success_returnsExpectedData() async throws {
		let response = anyPullSuccessResponse()
		let remote = SyncMockRemoteDataSource(pullResult: .success(response))
		let sut = makeSUT(remote: remote)
	...
	@Test(arguments: [401, 404, 500])
	func pull_whenThrowsErrorResponse_returnsErrorState(statusCode: Int) async throws {
		let expectedError = ErrorResponse(success: false, statusCode: statusCode, message: "Request failed", errors: nil)
		let remote = SyncMockRemoteDataSource(pullResult: .failure(expectedError))
		let sut = makeSUT(remote: remote)
	...
	@Test func pull_whenThrowsGenericError_returnsErrorState() async throws {
		let dummyError = NSError(domain: "TestError", code: 999)
		let remote = SyncMockRemoteDataSource(pullResult: .failure(dummyError))
		let sut = makeSUT(remote: remote)
